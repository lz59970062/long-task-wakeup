# Committee Review: LTC 0.6.5 independent test template

Date: 2026-09-14
Outcome: Approved after validation

## Expert panel

| Expert | Lens | Final position |
| --- | --- | --- |
| Ada / api_review | Compatible CLI and minimal design | Approve after validation |
| Lin / test_review | Independent behavioral oracle | Approve after validation |
| Sam / reliability_review | Durable snapshots and honest handoffs | Approve after validation |

## Findings and resolution

The committee supports an explicit `--template test` flag on the existing child-agent command. Codex uses Luna/max by default; child model options never alter the parent's callback configuration. Claude retains its own model configuration and rejects the Codex-only effort flag. Existing generic prompts remain literal.

The central requirement is an independent oracle, not merely a different model. The template asks for requirements before implementation, plausible faults caught, test-only changes, and a truthful unexecuted handoff. The parent reviews and executes tests, diagnoses failures and does not weaken assertions for a green result. Shared-workspace prompt constraints are documented rather than presented as isolation.

Task submission freezes expanded instructions and explicit/template-resolved configuration; callback metadata preserves that provenance. Nonzero/empty/partial authoring is explicitly distinguished from passing tests. Inherited CLI defaults remain external and are documented accordingly.

## Validation

109 existing regressions and 9 independently authored template tests pass. Lin wrote the new tests; the parent executed them. A fake child subprocess verifies queue-to-worker-to-callback success and failure, including unchanged saved configuration after template defaults change. Wheel build, temporary installation, and template dry-run outside the source tree pass. Local CLI help/model catalog verify the flag form and Luna/max combination; no real model completion was requested.

The initial baseline test attempt hit sandbox restrictions on real-user locks and local sockets. Re-running with isolated temporary lock storage and local socket permissions passed; no application code was changed to suppress these failures.

## Remaining boundaries

No dissent. Generated test quality and child compliance still require parent inspection. Template authoring success never certifies product correctness. No remote package publication or installed-daemon upgrade is part of this code validation.
