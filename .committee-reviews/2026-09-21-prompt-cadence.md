...
- Ada: API clarity and predictable interval semantics.
- Lin: independent behavioral testing and content preservation.
- Sam: concurrency, crash consistency and delivery safety.

## Result

Use `ltc prompt-policy --system-every 4 --user-every 3` to configure separate reminder intervals. First callback shows both; the cadence is anchored at ordinal 1. Settings live outside the queue; counters are queue/agent/session scoped. Retries retain their original ordinal and decisions even after policy edits or restarts. User hook content is reread on due attempts.

Generated callbacks contain full and compact alternatives. Compact text preserves task data, custom messages, recovery/template instructions, full routing constraints and an executable ACK command. Prompt selection happens in the locked delivery worker and updates the shared payload for both Desktop and CLI. The skill's core rules explain that omitted prose does not remove responsibilities.

A separate stable lock serializes an atomic ledger containing both counter and callback allocations. Corrupt configuration or history causes full-reminder fallback rather than blocking the callback or resetting history. Already acknowledged/canceled worker startups do not count. Legacy queued records without compact text retain their standard body; --last retains full reminders because the actual conversation is unknown.

## Validation

145 tests pass; 14 cadence tests independently authored by Lin and executed by the parent. They cover schedules, retries, policy updates, concurrency for distinct and identical IDs, conservative fallback, preserved content, both transports and skipped startup. No live model was invoked. The initial focused run exposed a test fixture missing target_source; the fixture was corrected to match real request records without weakening content assertions.

## Accepted limits

The ledger retains allocation history for stable retries; no automatic pruning is introduced. The sequence counts first delivery attempts, not successful ACKs. Cadence reduces repeated text; it does not establish a guarantee about model memory.
